...
(define (parse sexp)
  (match sexp
    ...
    [(list 'seqn bfaes ...) (seqn (parse-sequence bfaes))]
...
(define (store-records rec ds st)
  (define (helper rec st addr)
    (type-case Records rec
      [mtRecord () (begin (display "store-records\n") (display addr) (display "\n") (display st) (display "\n")
                     (v*s (recordV addr) st))]
      [aRecord (id bfae rest)
               (begin; (display "store-records\n") (display st) (display "\n")
                 (local [(define interp-bfae (interp bfae ds st))
...
(interp-expr (app
              (fun 'r (seqn (aSeq (setbox (getField (id 'r) 'x) (num 2)) (aSeq (openbox (getField (id 'r) 'x)) (mtSeq)))))
              (rec (aRecord 'x (newbox (num 0)) (mtRecord)))))